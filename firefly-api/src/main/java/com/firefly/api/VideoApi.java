...
import com.firefly.service.VideoService;
...
public class VideoApi {
    ...
    /**
     * 视频在线播放
     * 前端 请求 ——> 后端  请求 ——> fdfs文件服务器
     * 后端对url包装，内部使用实际fdfs路径请求
     * url：就是对应file的追加文件路径（fdfs中的相对路径），由分片上传接口return
     * wKhHgWf6iSyEMFPUAAAAAJYIWsk400.mp4
     * 实际路径：http://<fdfs-nginx>/group1/M00/...
     * http://192.168.71.129:8888/group1/M00/00/00/wKhHgWf6iSyEMFPUAAAAAJYIWsk400.mp4
     * 每次只请求一定range的数据流(by slice)
     * 在 request headers中:range确定slice范围，表示此slice在byte流的开始与结束位置
     * 输出数据流在response
     *
     * service层：
     * Range 请求发送至 FastDFS HTTP 地址
     * FastDFS return对应range的字节。这样就实现了“在线播放（可拖动）
     */
